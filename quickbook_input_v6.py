...
def transform_excel_to_csv(input_file):
    ...
    for _, row in input_df.iterrows():
        ...
        if supplier_value.replace(".", "").isdigit():  # Check if numeric with decimal issue
            supplier_value = f"Driver {int(float(supplier_value))}"  # Convert properly
        else:
            supplier_value = f"Driver {supplier_value}"  # Keep non-numeric as is
        
        # Extract numbers from supplier column and check if any is greater than 10000
        numbers = [float(num) for num in supplier_value.split() if num.replace('.', '', 1).isdigit()]
