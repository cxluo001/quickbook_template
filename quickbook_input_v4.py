...
def transform_excel_to_csv(input_file):
    ...
    for _, row in input_df.iterrows():
        # Extract numeric portion safely and convert to integer if possible
        supplier_number = ''.join(filter(str.isdigit, str(row[supplier_col])))
        supplier_value = f"Driver {int(supplier_number)}" if supplier_number else "Driver Unknown"
