...
def transform_excel_to_csv(input_file):
    ...
    for _, row in input_df.iterrows():
        supplier_value = f"Driver {row[supplier_col]}"  # Keep original format
    ...
    # Ensure *Supplier column is always treated as a string to prevent .0 issue
    output_df["*Supplier"] = output_df["*Supplier"].astype(str)
    
    # Save as CSV in memory
    output_buffer = BytesIO()
    output_df.to_csv(output_buffer, index=False, quoting=1)  # quoting=1 ensures consistent string format
