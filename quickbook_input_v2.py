...
def transform_excel_to_csv(input_file):
    ...
    supplier_col = "直营\n(Driver)"
    skip_col = "总收入含税 \n(Total Income w Tax)"  # Replace with the actual column name
    amount_cols = [
    ...
    input_df = input_df[
        (input_df[supplier_col].notna()) & 
        (~input_df[supplier_col].astype(str).str.contains("Total", case=False)) &
        (input_df[skip_col] != 0)
    ]
    ...
    for _, row in input_df.iterrows():
        supplier_value = f"Driver {row[supplier_col]}"  # Format supplier name
